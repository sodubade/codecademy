...
function loadDatabase(){
  try{
    return yaml.safeLoad(fs.readFileSync('./scoopDB.yaml', 'utf8'));
  } catch (e){
    console.log('loadDatabase Error: ' + e);
  }
}

function saveDatabase(){
  try{
    const db = yaml.safeDump(database);
  } catch (e){
    console.log('Error while creating data: ' + e);
  }
  fs.writeFile('./scoopDB.yaml',db, function(err){
    if (err) throw err;
